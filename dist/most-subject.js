(function (global, factory) {
	...
	(factory((global.mostSubject = global.mostSubject || {})));
...
function asSync(stream) {
    return new SyncSubject(new MulticastSource(stream.source));
}
var SyncSubject = (function (_super) {
    ...
    SyncSubject.prototype.next = function (value) {
        this.source.event(defaultScheduler.now(), value);
        return this;
    };
    SyncSubject.prototype.error = function (err) {
        this.source.error(defaultScheduler.now(), err);
        return this;
    };
    SyncSubject.prototype.complete = function (value) {
        this.source.end(defaultScheduler.now(), value);
        return this;
    };
    return SyncSubject;
}(Stream));

var next$1 = curry2(function next(value, subject) {
    return subject.next(value);
});

var error$2 = curry2(function error(err, subject) {
    return subject.error(err);
});

var complete = curry2(function complete(value, subject) {
    return subject.complete(value);
});

var hold = curry2(function hold(bufferSize, subject) {
    return new subject.constructor(new HoldSubjectSource(subject.source, bufferSize));
});
